...
class circleArc(SVGPathAtom):
  ...
  def toSVGString(self):
    return "a %f %f 0 %s %s %f %f"%(self.r, self.r, self.largeArc, self.sweepFlag, self.endPoint.x, self.endPoint.y)
  ...
  def addRoundedEdgeAt(self, radius, point, debug = False):
    ...
    for i in range(0, len(self)):
     ...
     if skip:
         skip = False
         continue
     if debug: inkex.debug("Test %s  (%.2f, %.2f) against (%.2f, %.2f)" % (repr(i), point.x, point.y, pos.x, pos.y))
...
#    self.insertPath(self.boxFrames(Point(40.0,40.0), 100.0, Direction.down, False))
  ...
  def drawMobileLoader(self):
    ...
    backRestStart = start.add(
            self.boxWidth + self.boxHeight + 1*self.hingeCircleFactor*self.thickness + self.boxWidth+self.thickness,0)
    ...
    shelfSupportHeight=(self.shelfHeight/math.cos(self.inclinationRad)-(self.supportDistance+self.thickness))/math.tan(self.inclinationRad)    
   ...
   # inkex.debug('boxFrame %s'%infoText)
    ...
    nrInOutFrames = int(math.floor((self.backRestWidth-(self.frameEdgesMin*2)-self.frameLength) / self.frameLength ))
    ...
    boxStart = backRestStart.add(remainder + self.frameLength/2, frameDepthWithOverhead + supportHeight/math.sin(self.inclinationRad))
    ...
    shelfFrameWidth=self.shelfHeight-self.thickness*(math.cos(self.inclinationRad)/math.sin(self.inclinationRad))    
    ...
    boxStart = testBoxStart.add(remainder + self.frameLength/2, 3*self.thickness + self.supportDistance + lochLength)
    for i in range(nrFrames):
      self.insertRect(boxStart, self.frameLength, self.thickness, 'blue')
      boxStart = boxStart.add(self.frameLength * 2, 0)
    
    # usbLochStart
    boxStart = testBoxStart.add(self.backRestWidth/2-self.usbWidth/2, 2*self.thickness+ lochLength - self.usbDepth/2)
    usbLoch = Path()
  ...
  def drawBox(self):
    ...
    infoText = self.boxType.description + " (generated on %s)  --- Width: %.2fmm, Depth: %.2fmm, Height: %.2fmm (Thickness: %.2fmm, frame length: %.2fmm)[shelfheight: %.2fmm]"%\
      (self.printDate(),self.boxWidth, self.boxDepth, self.boxHeight,self.thickness, self.frameLength, shelfHeight)
   # inkex.debug('boxFrame %s'%infoText)
...
# Shelf frames 
    if (self.boxType == shelvedBox) :
      ...
      nrInOutFrames = int(math.floor((self.boxHeight-(self.frameEdgesMin*2)-self.frameLength) / self.frameLength ))
      nrFrames = int(math.floor(nrInOutFrames / 2))
      ...
      for side in range(2) : # 0 is front,  1 is back
          for i in range(self.shelfcount - 1) :
            shelfFramesStart = start.add((i+1)*(shelfHeight + self.thickness),  (side * (self.boxDepth + self.boxHeight - 2*self.thickness)) )
      ...
      nrInOutFrames = int(math.floor((self.boxDepth-(self.frameEdgesMin*2)-self.frameLength) / self.frameLength ))
      nrFrames = int(math.floor(nrInOutFrames / 2))
      remainder = (self.boxDepth - nrFrames * self.frameLength * 2)/2 
      for i in range(self.shelfcount - 1) :
        shelfFramesStart = start.add((i+1)*(shelfHeight + self.thickness),  ( self.boxHeight - self.thickness) )
        # self.markPoints(shelfFramesStart, 'blue')
...
#    leftBoxStart = start.add(self.boxWidth + 2, self.boxHeight)
    ...
    leftHingecenter = leftBoxStart.add(self.boxHeight-self.thickness/2, self.boxDepth - self.thickness - self.thickness/2);
    if self.boxType.hasHinges() :
    ...
    if self.boxType.hasHinges() :
        ...
        leftPart.append(circleArc(outerRadius, Point(-(dx+self.thickness/2), dx+self.thickness/2 - self.thickness)))
...
# left part
    ...
    if self.boxType.hasHinges() :
        ...
        rightPart.append(circleArc(outerRadius, Point((dx+self.thickness/2), dx+self.thickness/2 - self.thickness)))
...
# TOP Part (only for hinged boxes)
    ...
    # make boxes for the mobile stand
        if self.boxType == mobileLoader :
            ...
            for supp in range(0, numberOfSupports):
                boxStart = topStart.add(self.thickness+remainder/2.0+self.distanceBetweenSupports * supp, (self.boxDepth - self.backRestWidth)/2.0)
    ...
    #         
                supportBoxStart = boxStart.add(self.thickness + lochLength + dxPlusd + self.supportDistance, remainderFrames + 0.5 * self.frameLength)
                for i in range(nrFrames):
                  ...
                  # usbLochStart
                lochBoxStart = boxStart.add(0.5*self.thickness + lochLength, self.backRestWidth/2-self.usbWidth/2)
...
# =============================================================================
  def drawShelves(self, start):
    for ii in range(self.shelfcount - 1) :
        shelfStart = start.add(self.boxWidth + self.boxHeight + self.thickness, self.thickness + ii*(self.boxDepth + self.thickness))
...
#      inkex.debug("Text: %s" % drw)
  ...
  def boxFrames(self, length, direction, inverse = False, depth = None) :
    ...
    frameMoveHalf = Point(direction['frameMove'][0]*self.frameLength/2.0, direction['frameMove'][1]*self.frameLength/2.0)
